...
fun getTagMap(arg: Tag): Map<String, Any?> {
  ...
  arg.techList.forEach { tech ->
    ...
    data[tech.lowercase(Locale.ROOT).split(".").last()] =
            when (tech) {
              ...
              Ndef::class.java.name ->
                      Ndef.get(arg).let {
                        mapOf(
                                ...
                                "cachedMessage" to
                                        if (it.cachedNdefMessage == null) null
                                        else getNdefMessageMap(it.cachedNdefMessage),
                                "type" to it.type
                        )
                      }
              // NdefFormatable or NfcBarcode
              else -> mapOf("identifier" to arg.id)
            }
  }
